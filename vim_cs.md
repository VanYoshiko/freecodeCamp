...
s - delete character and substitute text (same as cl)S - delete line and substitute text (same as cc)xp - transpose two letters (delete and paste)u - undoU - restore (undo) last changed lineCtrl + r - redo
. - repeat last command
}}}
...
{{{cutandpaste}}
ggyG - yank (copy) entire document.
:%y - yank entire document.
yy - yank (copy) a line
...
f - finds next available character (; goes to the next | , goes to previous)
t - places cursor before the character that is searched for.
}}}
...
Example for paste settings in .vimrc:
augroup filetype_md
    autocmd!
